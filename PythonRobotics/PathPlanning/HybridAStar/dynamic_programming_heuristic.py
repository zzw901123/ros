...
def calc_distance_heuristic(gx, gy, ox, oy, resolution, rr):
    ...
    motion = get_motion_model() #移动方向 与 代价

    open_set, closed_set = dict(), dict()
    open_set[calc_index(goal_node, x_w, min_x, min_y)] = goal_node #将目标节点放入到开放列表中
    priority_queue = [(0, calc_index(goal_node, x_w, min_x, min_y))]#创建优先队列 
    ...
    while 1:
        ...
        for i, _ in enumerate(motion):
            ...
            if n_id in closed_set: #close set   do not add
                continue

            if not verify_node(node, obstacle_map, min_x, min_y, max_x, max_y): #beyond the border or obs  do not add 
                continue

            if n_id not in open_set: # not in open set  add 
