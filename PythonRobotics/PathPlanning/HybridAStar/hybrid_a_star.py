...
def calc_motion_inputs():
    for steer in np.concatenate((np.linspace(-MAX_STEER, MAX_STEER,
                                             N_STEER), [0.0])): # N_STEER   step number not step length
        for d in [1, -1]:
            yield [steer, d]
...
def calc_next_node(current, steer, direction, config, ox, oy, kd_tree):
    ...
    for _ in np.arange(0, arc_l, MOTION_RESOLUTION):
        x, y, yaw = move(x, y, yaw, MOTION_RESOLUTION * direction, steer) # robot kinematics formula
        x_list.append(x)
        y_list.append(y)
        yaw_list.append(yaw)
...
def update_node_with_analytic_expansion(current, goal,
    ...
    if path:
        ...
        f_cost = currentim.cost + calc_rs_path_cost(path)
...
def hybrid_a_star_planning(start, goal, ox, oy, xy_resolution, yaw_resolution):
    ...
    heapq.heappush(pq, (calc_cost(start_node, h_dp, config),
                        calc_index(start_node, config))) # store cost and index
    final_path = None
