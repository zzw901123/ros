...
#enerate_trajectory(s,km,kp,k0)：s是运动的弧长，
# 由于是匀速模型，可以计算Δ t \Delta tΔt；
# 另外涉及到state lattice中的角速度模型部分，使用二次曲线，参数为二次曲线上的三个点[k0, km, kf ] 
# 将这三个点和时间拟合得到曲线后；向前积分得到整个轨迹上的状态点。
...
def generate_trajectory(s, km, kf, k0):
    ...
    #每ds生成一个状态数据 步长0.1m
    n = s / ds
    #匀速模型，经弧长s需要的时间 初始速度10.0 / 3.6 m/s
    time = s / v  # [s]
...
# 同generate_trajectory，但只返回整个轨迹上的最后一个状态点
