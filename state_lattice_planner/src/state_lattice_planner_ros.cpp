...
:local_nh("~")
{
    local_nh.param("HZ", HZ, {20}); // 控制频率
    local_nh.param("ROBOT_FRAME", ROBOT_FRAME, {"base_link"});
    local_nh.param("N_P", N_P, {10}); // x-y 位置的终端状态采样
    local_nh.param("N_H", N_H, {3}); // 方向的终端状态采样数
    local_nh.param("MAX_ALPHA", MAX_ALPHA, {M_PI / 3.0}); //最大终端状态采样方向
    local_nh.param("MAX_PSI", MAX_PSI, {M_PI / 6.0}); // 终端状态下的最大方向 
    local_nh.param("N_S", N_S, {1000}); // 全局引导采样数
    local_nh.param("MAX_ACCELERATION", MAX_ACCELERATION, {1.0}); // 最大加速度
    local_nh.param("TARGET_VELOCITY", TARGET_VELOCITY, {0.8}); // 机器人目标速度的最大速度
    local_nh.param("LOOKUP_TABLE_FILE_NAME", LOOKUP_TABLE_FILE_NAME, {std::string(std::getenv("HOME")) + "/lookup_table.csv"}); //查找表的绝对路径 
    local_nh.param("MAX_ITERATION", MAX_ITERATION, {100}); // 最大迭代次数
    local_nh.param("OPTIMIZATION_TOLERANCE", OPTIMIZATION_TOLERANCE, {0.1}); // 迭代终止条件
    local_nh.param("MAX_YAWRATE", MAX_YAWRATE, {0.8}); // 最大角速度
    local_nh.param("MAX_D_YAWRATE", MAX_D_YAWRATE, {2.0}); // 最大角加速度
    local_nh.param("MAX_WHEEL_ANGULAR_VELOCITY", MAX_WHEEL_ANGULAR_VELOCITY, {11.6}); // 轮子最大角速度
    local_nh.param("WHEEL_RADIUS", WHEEL_RADIUS, {0.125}); // 轮子半径
    local_nh.param("TREAD", TREAD, {0.5}); 
    local_nh.param("IGNORABLE_OBSTACLE_RANGE", IGNORABLE_OBSTACLE_RANGE, {1.0}); // 障碍物范围
    local_nh.param("VERBOSE", VERBOSE, {false});
    local_nh.param("CONTROL_DELAY", CONTROL_DELAY, {1}); //控制延迟
    ...
    planner.set_sampling_params(StateLatticePlanner::SamplingParams(N_P, N_H, MAX_ALPHA, MAX_PSI)); //TODO  采样参数 MAX_PSI？
    planner.set_optimization_params(MAX_ITERATION, OPTIMIZATION_TOLERANCE); // 设置迭代参数
    planner.set_vehicle_params(WHEEL_RADIUS, TREAD); // 轮子的参数
    planner.set_motion_params(MAX_ACCELERATION, MAX_YAWRATE, MAX_D_YAWRATE); // 运动参数
    planner.set_target_velocity(TARGET_VELOCITY); // 目标速度

    velocity_pub = nh.advertise<geometry_msgs::Twist>("/cmd_vel", 1); //发布
    ...
    local_goal_sub = nh.subscribe("/local_goal", 1, &StateLatticePlannerROS::local_goal_callback, this); // 订阅
    ...
    planner.load_lookup_table(LOOKUP_TABLE_FILE_NAME); //获取查询表
}
...
void StateLatticePlannerROS::process(void)
{
    ...
    while(ros::ok()){
        ...
        if(local_goal_subscribed){ //获取到local_goal(相对于世界坐标系)
            try{
                listener.transformPose(ROBOT_FRAME, ros::Time(0), local_goal, local_goal.header.frame_id, local_goal_base_link); //转换到base_link坐标系下
