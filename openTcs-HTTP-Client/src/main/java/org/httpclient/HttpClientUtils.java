...
public class HttpClientUtils {
     ...
     * @param decodeCharset 解码字符集,解析响应数据时用到,为null时默认采用UTF-8解码
     * @return 远程主机响应正文
     * @description 该方法会自动关闭连接, 释放资源
     */
     ...
     * @author cakin
     * @date 2020/8/20
     * @description: 该方法为 public static String sendPostRequest(String reqURL, String sendData, boolean isEncoder, String encodeCharset, String decodeCharset)的简化方法
     * 该方法在对请求数据的编码和响应数据的解码时,所采用的字符集均为UTF-8
     * 当isEncoder=true时,其会自动对sendData中的[中文][|][ ]等特殊字符进行URLEncoder.encode(string,"UTF-8")
     */
     ...
     *
     * @param reqURL        请求地址
     * @param sendData      请求参数,若有多个参数则应拼接成param1=value1&param2=value2&param3=value3的形式后,传入该参数中
     * @param isEncoder     请求数据是否需要encodeCharset编码,true为需要
     * @param encodeCharset 编码字符集,编码请求数据时用之,其为null时默认采用UTF-8解码
     * @param decodeCharset 解码字符集,解析响应数据时用之,其为null时默认采用UTF-8解码
     * @return 远程主机响应正文
     * @description 该方法会自动关闭连接, 释放资源
     * 当 isEncoder=true 时,其会自动对 sendData 中的[中文][|][ ]等特殊字符进行 URLEncoder.encode(string,encodeCharset)
     */
     ...
     * @author cakin
     * @date 2020/8/20
     * @description: 该方法为 sendPostSSLRequest(String,Map<String,String>,String,String) 方法的简化方法
     * 该方法在对请求数据的编码和响应数据的解码时,所采用的字符集均为UTF-8
     * 该方法会自动对 params 中的[中文][|][ ]等特殊字符进行 URLEncoder.encode(string,"UTF-8")
     */
     ...
     * @return 远程主机响应正文`HTTP状态码
     * "SUCCESS`200"：
     * 若通信过程中发生异常则返回 "Failed`HTTP状态码"
     * "Failed`500"
     * @description 若发送的 params 中含有中文,记得按照双方约定的字符集将中文 URLEncoder.encode(string,encodeCharset)
     ...
     * 若通信过程中发生异常则返回 "Failed`HTTP状态码"
     * "Failed`500"
     * @description 若发送的 sendData 中含有中文,记得按照双方约定的字符集将中文 URLEncoder.encode(string,encodeCharset)
     * 本方法默认的连接超时时间为30秒,默认的读取超时时间为30秒
     */
     ...
     * @param ca 证书
     * @param caPwd 证书密码
     * @return String 主机返回数据
     * @throws Exception
     */
    public static String httpsPostByCa(String url, String data, String ca, String caPwd) throws Exception {
        KeyStore keyStore = KeyStore.getInstance("PKCS12");
        FileInputStream is = new FileInputStream(new File(ca));
        try {
            keyStore.load(is, caPwd.toCharArray());// 这里写密码..默认是你的MCHID
        } finally {
            is.close();
        }
        // Trust own CA and all self-signed certs
        SSLContext sslcontext = SSLContexts.custom()
                .loadKeyMaterial(keyStore, caPwd.toCharArray())// 这里也是写密码的
                .build();
